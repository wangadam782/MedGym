...
## Step 4: Inspect evaluation metrics

The cluster offline evaluator writes one `per_patient_metrics.csv` per scope and
method under the selected `--eval_root`. For example, the released cohort_7
policies can be evaluated with the command in the top-level README and inspected at
`results/offline_eval/cohort_7/ind/cql/summary/per_patient_metrics.csv`.

The earlier plotting command for Table 7 and Figures 11--14 referred to a script
that is not part of this repository. Those figures are not generated by the
current offline evaluation pipeline.
